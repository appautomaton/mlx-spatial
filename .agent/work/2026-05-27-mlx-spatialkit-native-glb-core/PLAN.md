...
**Status:** complete
**Evidence:** Added `export_pixal3d_glb(...)` as the package-level real fixture export orchestrator: it loads `shape_decoder_fields.npz` and `texture_decoder_pbr.npz`, runs native FlexiDualGrid extraction, native mesh metrics/cleanup/simplification, native face-atlas UVs, Metal PBR texture bake, native GLB writing, and writes `diagnostics.json` beside `model.glb` under `/tmp`. Diagnostics now include stage timings, current/peak RSS samples, mesh counts, cleanup/simplification stats, UV stats, texture coverage stats, source metadata, settings, and GLB bytes. Added opt-in heavy real fixture coverage plus fast guard-validation coverage; default package tests now exclude `heavy`. `UV_CACHE_DIR=/tmp/mlx-spatialkit-uv-cache uv run --directory packages/mlx-spatialkit --reinstall-package mlx-spatialkit pytest tests -q` passed with `38 passed, 1 deselected`; `UV_CACHE_DIR=/tmp/mlx-spatialkit-uv-cache uv run --directory packages/mlx-spatialkit --reinstall-package mlx-spatialkit pytest tests/test_real_pixal3d_export.py -q -m heavy` passed with `1 passed, 1 deselected`; `git diff --check` passed; `xcrun metal -v && UV_CACHE_DIR=/tmp/mlx-spatialkit-uv-cache uv build packages/mlx-spatialkit --out-dir /tmp/mlx-spatialkit-dist --clear && ...` passed, printed `artifact clean`, and asserted the wheel includes `mlx_spatialkit/texture_bake.metallib`. Read-only spec review returned `APPROVED`; quality review requested explicit guard validation for `grid_size` / `max_texture_pixels`, the fix was added and re-review returned `APPROVED`.
**Risks / next:** Slice 7 proves a valid GLB from the real 1024 Pixal3D decoded fixture through the new native path, but visual quality is still bounded by first-pass face-atlas UVs, nearest sparse-voxel sampling, and native simplification. Slice 8 must integrate this backend into `mlx-spatial` behind an explicit optional boundary and document the `/tmp`/heavy-fixture workflow.
...
**Status:** complete
**Evidence:** Added explicit optional Pixal3D GLB backend selection: `internal` remains the default and `spatialkit` lazily imports `mlx_spatialkit.export_pixal3d_glb` only when requested. Missing `mlx_spatialkit` falls back to internal export and records `glb_export_backend_requested`, `glb_export_backend_used`, and fallback reason; installed-backend runtime failures still return structured `mesh-export` blockers. Root CLI and script help now expose `--glb-export-backend {internal,spatialkit}` and `--glb-diagnostics-path`; public exports include the backend constants. Pipeline tests cover spatialkit handoff, missing-package fallback, backend validation, diagnostics artifact inclusion, and stable `mesh_export` trace keys. Docs now explain decoded NPZ -> GLB, spatialkit diagnostics, heavy fixture expectations, `/tmp` output policy, optional fallback behavior, and artifact cleanliness boundaries. `UV_CACHE_DIR=/tmp/mlx-spatial-uv-cache uv run pytest tests/test_pixal3d_pipeline.py tests/test_pixal3d_export.py tests/test_pixal3d_assets.py tests/test_pixal3d_parity.py -q` passed with `46 passed`; CLI help checks for both `mlx-spatial-pixal3d generate --help` and `python scripts/pixal3d/generate.py --help` showed the new options; `git diff --check` passed; root `uv build` artifact check passed and printed `root artifact clean`; `xcrun metal -v && UV_CACHE_DIR=/tmp/mlx-spatialkit-uv-cache uv build packages/mlx-spatialkit --out-dir /tmp/mlx-spatialkit-dist --clear && ...` passed, printed `spatialkit artifact clean`, and asserted the wheel includes `texture_bake.metallib`. Read-only spec review returned `APPROVED`; quality review requested diagnostics artifact inclusion and stable `mesh_export` keys, the fixes were added and re-review returned `APPROVED`.
**Risks / next:** The full native GLB objective is implemented and verified, but spatialkit visual quality is still first-pass face-atlas/nearest-voxel quality. Final verification should rerun aggregate checks and ensure no generated caches or `/tmp` artifacts are staged.
