# mlx-spatialkit Native Chart Minimum Padding Gate Plan

## Goal

Execute `.agent/work/2026-05-27-mlx-spatialkit-native-chart-minimum-padding-gate/SPEC.md`: tighten native-chart default padding to `0.005` and verify real fixture occupancy progress.

## Architecture Approach

Change only the Pixal3D backend default for `uv_backend="native-chart"`; keep face-atlas padding, explicit caller padding, UV generation, texture bake, and readiness semantics unchanged. Use existing diagnostics to prove the new default and measured occupancy change.
...
### Slice 1: Native-Chart Minimum Padding Default

**Objective:** Set native-chart default padding to `0.005` and prove reference-target native-chart occupancy advances.

**Acceptance criteria:**
- Resolver/unit and real-fixture tests assert `tile_padding=0.005` with `backend_default:native-chart`.
- Reference-target native-chart heavy gate reports UV-surface occupancy above `0.5597553253173828` or xatlas utilization ratio above `0.6736181097830843`.
...
**Status:** complete
**Evidence:** resolver guard passed; reference-target native-chart heavy gate passed. Latest `/tmp` diagnostics report `tile_padding=0.005`, `tile_padding_source=backend_default:native-chart`, UV-surface occupancy `0.5693244934082031` vs Phase 29 `0.5597553253173828`, xatlas utilization ratio `0.685133792850289` vs Phase 29 `0.6736181097830843`, visual comparison passing, and `parity_ready=false`.
...
- Docs mention `0.005` native-chart default padding without claiming xatlas equivalence.
...
**Status:** complete
**Evidence:** `git diff --check` passed; explicit 1M/4096 native-chart heavy gate passed in `44.57s` with `tile_padding=0.005`, upstream readiness true, native-chart quality-ready, UV-surface occupancy `0.5488805174827576`, and xatlas utilization ratio `0.660531199902127`; package tests passed `64 passed, 7 deselected`; root Pixal3D tests passed `35 passed`; `/tmp` build produced wheel and sdist; artifact inspection reported wheel `bad 0` and sdist `bad 0`.
