...
from mlx_spatial.spatialkit import export_pixal3d_glb, metal_device_available
...
from mlx_spatial.spatialkit.export import (
    NativeGlbArtifact,
...
def test_export_pixal3d_glb_real_decoded_fixture_writes_glb_and_diagnostics(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "real-pixal3d-export"
...
def test_export_pixal3d_glb_udf_remesh_closes_topology_and_matches_reference_mechanism(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "remesh-export"
    result = export_pixal3d_glb(
        fixture,
        output_dir,
        texture_size=1024,
    ...
    # The official UDF remesh closes open boundaries by building an offset
    # shell. It remains explicitly non-single-layer while matching the
    # reference application's remesh mechanism.
    remesh_stage = diagnostics["stages"]["remesh"]
    assert remesh_stage["stats"]["backend"] == "cpu-narrow-band-udf-double-cover-dc"
    assert remesh_stage["stats"]["surface_representation"] == "udf-offset-double-cover"
    assert remesh_stage["stats"]["single_surface_ready"] is False
    assert remesh_stage["stats"]["active_voxels"] > 0
    remesh_metrics = diagnostics["stages"]["remesh_metrics"]["metrics"]
    ...
    simplify_stats = diagnostics["stages"]["simplify_mesh"]["stats"]
    assert simplify_stats["remesh_reference_mechanism_ready"] is True
    assert simplify_stats["remesh_equivalence_status"] == "cumesh-reference-mechanism-matched"
    topology = diagnostics["quality"]["topology_blocker_map"]
    narrow_band = topology["classes"]["missing_narrow_band_remesh"]
    assert narrow_band["present"] is False
    assert narrow_band["backend"] == "native-udf-double-cover-dc"
...
def test_export_pixal3d_glb_native_chart_backend_writes_real_fixture(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "native-chart-export"
    result = export_pixal3d_glb(
        fixture,
        output_dir,
        texture_size=1024,
...
def test_export_pixal3d_glb_native_chart_violin_preprocessed_black_fixture(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "violin-black-native-chart-export"
    result = export_pixal3d_glb(
        fixture,
        output_dir,
        texture_size=1024,
    ...
    assert visual["rendered_visual_blockers"] == ["spatial_render_proof", "boundary_open_chains"]
...
def test_export_pixal3d_glb_reference_target_preset_reports_thresholds(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "reference-target-export"
    result = export_pixal3d_glb(
        fixture,
        output_dir,
    ...
    assert visual["rendered_visual_checks"]["spatial_render_proof"]["passed"] is False
    ...
    assert "not_spatial_rendered_visual_proof" in visual["deferred_parity_boundaries"]
    visual_artifacts = visual["artifacts"]
    assert Path(visual_artifacts["report_json"]).is_file()
    assert Path(visual_artifacts["preview_html"]).is_file()
    assert Path(visual_artifacts["candidate_base_color_png"]).is_file()
...
def test_export_pixal3d_glb_reference_target_native_chart_backend_reports_readiness(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "native-chart-reference-target-export"
    result = export_pixal3d_glb(
        fixture,
        output_dir,
        quality_preset="reference-target",
...
def test_export_pixal3d_glb_reference_target_4096_texture_reports_texture_resolution_gate(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "reference-target-4096-export"
    result = export_pixal3d_glb(
        fixture,
        output_dir,
        quality_preset="reference-target",
...
def test_export_pixal3d_glb_upstream_settings_passes_readiness_gate(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "upstream-settings-export"
    result = export_pixal3d_glb(
        fixture,
        output_dir,
        quality_preset="reference-target",
...
def test_export_pixal3d_glb_native_chart_upstream_settings_passes_readiness_gate(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "native-chart-upstream-settings-export"
    result = export_pixal3d_glb(
        fixture,
        output_dir,
        quality_preset="reference-target",
    ...
    assert visual["deferred_parity_boundaries"] == [
        "not_xatlas_chart_parity",
        "not_1m_face_export_setting_parity",
        "not_spatial_rendered_visual_proof",
    ]
...
def test_export_pixal3d_glb_simplify_backend_validator(tmp_path) -> None:
    # F1: only explicit QEM experiment backends are public overrides.
    assert _resolve_simplify_backend(None) is None
    assert _resolve_simplify_backend("qem") == "qem"
    assert _resolve_simplify_backend("QEM") == "qem"
    assert _resolve_simplify_backend("mlx-qem") == "mlx-qem"
    assert _resolve_simplify_backend("MLX-QEM") == "mlx-qem"
    assert _resolve_simplify_backend("single-layer-qem") == "single-layer-qem"
    assert _resolve_simplify_backend("SINGLE-LAYER-QEM") == "single-layer-qem"
    assert _resolve_simplify_backend("single-layer-mlx-qem") == "single-layer-mlx-qem"
...
def test_export_pixal3d_glb_simplify_backend_mlx_qem_requires_remesh(tmp_path) -> None:
    decoded_dir = tmp_path / "decoded"
    decoded_dir.mkdir()

    with pytest.raises(ValueError, match="simplify_backend='mlx-qem'.*remesh=True"):
        export_pixal3d_glb(decoded_dir, tmp_path / "out", simplify_backend="mlx-qem")


def test_export_pixal3d_glb_single_layer_qem_rejects_remesh(tmp_path) -> None:
    decoded_dir = tmp_path / "decoded"
    decoded_dir.mkdir()

    with pytest.raises(ValueError, match="single-layer-qem.*remesh=False.*single-layer"):
        export_pixal3d_glb(
            decoded_dir,
            tmp_path / "out",
            simplify_backend="single-layer-qem",
            remesh=True,
            remesh_repair_nonmanifold=True,
        )

    with pytest.raises(ValueError, match="single-layer-mlx-qem.*remesh=False.*single-layer"):
        export_pixal3d_glb(
            decoded_dir,
            tmp_path / "out",
    ...
    production = _export_quality_summary(
        {
            ...
            "final_faces": 212_542,
                ...
                "remesh_backend": "native-udf-double-cover-dc",
                "remesh_surface_representation": "udf-offset-double-cover",
                "remesh_single_surface_ready": False,
                "remesh_reference_mechanism_ready": True,
            },
        {"export_blocking_reasons": []},
        {
            "coverage_ratio": 0.75,
...
def test_reference_stage_contract_qem_simplify_reference_is_true() -> None:
    ...
    from mlx_spatial.spatialkit.export import _pixal3d_reference_stage_contract
    ...
    qem_stats_remesh_cleared = {
        ...
        "qem_equivalence_status": "edge-collapse",
        "remesh_backend": "native-udf-double-cover-dc",
        "remesh_equivalence_status": "cumesh-reference-mechanism-matched",
        "remesh_surface_representation": "udf-offset-double-cover",
        "remesh_single_surface_ready": False,
        "remesh_reference_mechanism_ready": True,
...
def test_reference_stage_contract_requires_both_mlx_qem_hole_fill_boundaries() -> None:
    from mlx_spatial.spatialkit.export import _pixal3d_reference_stage_contract
    ...
    stats = {
        ...
        "pre_qem_hole_fill_backend": "native-cpp",
        "pre_qem_hole_fill_algorithm": "cumesh-perimeter-centroid-fan",
        "pre_qem_hole_fill_max_perimeter": 0.03,
        "post_qem_hole_fill_backend": "native-cpp",
        "post_qem_hole_fill_algorithm": "cumesh-perimeter-centroid-fan",
        "post_qem_hole_fill_max_perimeter": 0.03,
        "post_qem_hole_fill_residual_clean_boundary_loops": 7,
    }
    contract = _pixal3d_reference_stage_contract(
        stats,
        {"backend": "face-atlas"},
    ...
    stage = contract["stages"]["cumesh_hole_fill_cleanup"]
    assert stage["status"] == "reference_matched"
    assert stage["spatialkit_backend"]["post_qem_residual_clean_boundary_loops"] == 7

    incomplete = _pixal3d_reference_stage_contract(
        {**stats, "pre_qem_hole_fill_algorithm": "unknown"},
        {"backend": "face-atlas"},
        {},
        None,
        quality_preset="reference-target",
    )
    assert incomplete["stages"]["cumesh_hole_fill_cleanup"]["status"] == "heuristic_quarantined"
...
def test_export_quality_summary_qem_stats_clears_qem_blocker_and_keeps_remesh_blocker() -> None:
    ...
    from mlx_spatial.spatialkit.export import _export_quality_summary
...
def test_production_equivalence_summary_keeps_parity_boundaries_strict() -> None:
    ...
    measured_xatlas = _production_equivalence_summary(
        {
            "artifact_ready": True,
            "production_quality_ready": True,
            "reference_stage_contract": {"passed": True},
    ...
    assert "not_xatlas_chart_parity" not in measured_xatlas["remaining_parity_boundaries"]
    assert measured_xatlas["remaining_parity_boundaries"] == (
        "not_1m_face_export_setting_parity",
    )
...
def test_export_pixal3d_glb_qem_input_prep_real_fixture_manifold_and_bounded(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "qem-input-prep-export"
    result = export_pixal3d_glb(
        fixture,
        output_dir,
...
def test_export_pixal3d_glb_qem_two_fixture_main_manifold_and_beats_clustering(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "qem-main-proof"
...
def test_export_pixal3d_glb_qem_two_fixture_violin_manifold_and_beats_clustering(tmp_path: Path) -> None:
    ...
    output_dir = tmp_path / "qem-violin-proof"
...
def _assert_chart_growth_parity_against_oracle(fixture: Path, anchor_name: str) -> dict:
    ...
    (tests/spatialkit/tools/gen_uv_oracle_anchors.py), then:
    ...
    from mlx_spatial.spatialkit._native import compute_uv_charts, grow_uv_charts
...
def _assert_parameterization_invariants_and_stretch_parity(
    ...
    from mlx_spatial.spatialkit._native import (
        compute_uv_charts,
        grow_uv_charts,
...
def test_uv_backend_validator_accepts_reference_backend() -> None:
    from mlx_spatial.spatialkit.export import _resolve_pixal3d_uv_backend, _resolve_tile_padding
...
def test_inpaint_oracle_parity_against_cv2_telea() -> None:
    ...
    from mlx_spatial.spatialkit import telea_inpaint
    ...
    anchors = json.loads(anchors_path.read_text())
    cache_value = os.environ.get("MLX_SPATIAL_INPAINT_ORACLE_CACHE")
    if not cache_value and os.environ.get("MLX_SPATIAL_TEST_SCRATCH"):
        cache_value = str(Path(os.environ["MLX_SPATIAL_TEST_SCRATCH"]) / "cache/inpaint-oracle")
    if not cache_value:
        pytest.skip(
            "set MLX_SPATIAL_TEST_SCRATCH or MLX_SPATIAL_INPAINT_ORACLE_CACHE "
            "to a task-specific scratch location"
        )
    ...
    if checked == 0:
        pytest.skip(
            f"no inpaint oracle cache present under {cache_dir}; "
            "regenerate via tests/spatialkit/tools/gen_inpaint_oracle_anchors.py"
        )
...
def test_export_telea_postprocess_real_fixture_paints_gutter_without_black_seam(tmp_path: Path) -> None:
    ...
    out_dir = tmp_path / "telea-postprocess"
    result = export_pixal3d_glb(
        fixture,
        out_dir,
        texture_size=1024,
...
def test_export_4096_telea_reference_path_within_budgets(
    tmp_path: Path,
    fixture_key: str,
    fixture_subpath: str,
) -> None:
    ...
    out_dir = tmp_path / f"4096-telea-budget-{fixture_key}"
