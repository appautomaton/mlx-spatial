...
under ``$MLX_SPATIAL_TEST_SCRATCH/cache/inpaint-oracle/`` (raw channels, mask,
and cv2 outputs) that the heavy parity test consumes.
...
For each cached Pixal3D fixture this script:
  1. Phase A (PROJECT venv, this process): runs ``export_pixal3d_glb`` with
     ...
     mask + our outputs to a task-local scratch NPZ.
  2. Phase B (ORACLE venv subprocess, under the task scratch root, pinned
     opencv-python-headless): runs ``cv2.inpaint(..., INPAINT_TELEA)`` per
     channel group and dumps the cv2 outputs to a task-local NPZ.
...
Usage (from the repository root):
    uv run python tests/spatialkit/tools/gen_inpaint_oracle_anchors.py
    uv run python tests/spatialkit/tools/gen_inpaint_oracle_anchors.py --fixtures main
    uv run python tests/spatialkit/tools/gen_inpaint_oracle_anchors.py --texture-size 512

Oracle venv bootstrap (one-time; the PROJECT venv must NOT have cv2):
    python3 -m venv "$MLX_SPATIAL_TEST_SCRATCH/cache/inpaint-oracle-venv" && \\
        "$MLX_SPATIAL_TEST_SCRATCH/cache/inpaint-oracle-venv/bin/pip" install opencv-python-headless==4.10.0.84 numpy

Internal phase-B invocation (dispatched automatically, do not run by hand):
    "$MLX_SPATIAL_TEST_SCRATCH/cache/inpaint-oracle-venv/bin/python" tests/spatialkit/tools/gen_inpaint_oracle_anchors.py \\
...
ANCHORS_PATH = TEST_ROOT / "spatialkit" / "data" / "inpaint_oracle_anchors.json"
...
def main() -> int:
    ...
    from mlx_spatial.spatialkit import export_pixal3d_glb, metal_device_available, telea_inpaint

    if not metal_device_available():
        print("Metal device unavailable; cannot bake fixtures.", file=sys.stderr)
        return 2
    oracle_python = (
        Path(args.oracle_python)
        if args.oracle_python
        else SCRATCH_ROOT / "cache/inpaint-oracle-venv/bin/python"
    )
    if not oracle_python.exists():
        print(f"oracle venv missing: {oracle_python}\nbootstrap: {_oracle_bootstrap_recipe()}", file=sys.stderr)
        return 2
    ...
    for key in args.fixtures:
        ...
        if not fixture.exists():
            ...
            continue
        out_dir = SCRATCH_ROOT / "outputs" / f"inpaint-oracle-export-{key}"
...
def _configure_scratch_paths() -> None:
    ...
    CACHE_DIR = SCRATCH_ROOT / "cache/inpaint-oracle"
    LOG_PATH = SCRATCH_ROOT / "logs/inpaint-oracle-anchors.log"
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    LOG_PATH.parent.mkdir(parents=True, exist_ok=True)


def _oracle_bootstrap_recipe() -> str:
    venv = SCRATCH_ROOT / "cache/inpaint-oracle-venv"
    return (
        f"python3 -m venv {venv} && {venv / 'bin/pip'} install "
        f"opencv-python-headless=={ORACLE_CV2_VERSION} numpy"
    )
