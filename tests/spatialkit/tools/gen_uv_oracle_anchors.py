...
For both cached Pixal3D fixtures this script:
  1. Phase A (PROJECT venv, this process): rebuilds the QEM-decimated 50k mesh
     ...
     mlx_spatial.spatialkit._native.compute_uv_charts with production knobs, and dumps
     per-cluster submeshes plus the whole mesh to a task-local scratch NPZ.
  2. Phase B (ORACLE venv subprocess, under the task scratch root, pip
     ...
     uvs/faces/vmapping/per-face chart ids back to a task-local NPZ.
...
Usage (from the repository root):
    ...
    uv run python tests/spatialkit/tools/gen_uv_oracle_anchors.py --reuse-cache
    uv run python tests/spatialkit/tools/gen_uv_oracle_anchors.py --reuse-cache --fixtures main

--reuse-cache reuses the cached QEM meshes under the task scratch cache (the
...
written under the task scratch root; all heavy intermediates stay there.

Oracle venv bootstrap (one-time):
    python3 -m venv "$MLX_SPATIAL_TEST_SCRATCH/cache/uvoracle-venv" && \\
        "$MLX_SPATIAL_TEST_SCRATCH/cache/uvoracle-venv/bin/pip" install xatlas==0.0.11 numpy

Internal phase-B invocation (dispatched automatically, do not run by hand):
    "$MLX_SPATIAL_TEST_SCRATCH/cache/uvoracle-venv/bin/python" tests/spatialkit/tools/gen_uv_oracle_anchors.py \
...
ANCHORS_PATH = TEST_ROOT / "spatialkit" / "data" / "uv_oracle_anchors.json"
...
# Must not import mlx_spatial.spatialkit; this is the isolated xatlas oracle.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Phase A: runs under the unified project environment.
# ---------------------------------------------------------------------------

def _spatialkit_version() -> str:
    """Installed mlx-spatial version, for tagging the QEM mesh cache."""
    try:
        from importlib.metadata import version  # noqa: PLC0415

        return version("mlx-spatial")
...
def build_qem_mesh(name: str, fixture: Path, reuse_cache: bool) -> tuple[np.ndarray, np.ndarray]:
    ...
    if reuse_cache:
        ...
        if cache_path.exists():
            ...
            log(f"[{name}] WARNING: reusing legacy QEM cache WITHOUT a package-version tag "
                ...
                f"under mlx-spatial SpatialKit {pkg_version}.")
    ...
    from mlx_spatial.spatialkit.mesh import (  # noqa: PLC0415  (project venv only)
        clean_mesh,
...
def _metrics_block(
    ...
    from mlx_spatial.spatialkit._native import uv_quality_metrics  # noqa: PLC0415
...
def process_fixture(name: str, fixture: Path, oracle_python: str, reuse_cache: bool) -> dict:
    from mlx_spatial.spatialkit._native import compute_uv_charts  # noqa: PLC0415
    ...
    scratch_in = SCRATCH_ROOT / "artifacts/uv-oracle" / f"{name}-in.npz"
    scratch_out = SCRATCH_ROOT / "artifacts/uv-oracle" / f"{name}-out.npz"
    scratch_in.parent.mkdir(parents=True, exist_ok=True)
...
def run_phase_a(args: argparse.Namespace) -> None:
    from mlx_spatial.spatialkit import metal_device_available  # noqa: PLC0415
    ...
    if not Path(args.oracle_python).exists():
        raise SystemExit(
            ...
            f"Bootstrap it first: {_oracle_bootstrap_recipe()}"
        )
    ...
    anchors = {
        "generated_with": {
            ...
            "numpy_version_oracle": oracle_versions.get("numpy_version_oracle", "unknown"),
            "numpy_version_project": np.__version__,
            "oracle_python": "external-isolated-environment",
            "generated_at": datetime.datetime.now(datetime.timezone.utc).isoformat(),
            "command": "uv run python tests/spatialkit/tools/gen_uv_oracle_anchors.py",
...
def main() -> None:
    ...
    parser.add_argument(
        "--reuse-cache",
        action="store_true",
        help="reuse cached QEM meshes under the task-specific scratch root",
    )
    parser.add_argument(
        "--oracle-python",
        help="oracle venv python with pip xatlas; defaults below MLX_SPATIAL_TEST_SCRATCH",
    )
    args = parser.parse_args()
    if args.phase_b:
        run_phase_b(args.phase_b[0], args.phase_b[1])
        return
    _configure_scratch_paths()
    if args.oracle_python is None:
        args.oracle_python = str(SCRATCH_ROOT / "cache/uvoracle-venv/bin/python")
    if not [n.strip() for n in args.fixtures.split(",") if n.strip()]:
        parser.error("--fixtures must name at least one fixture")
    run_phase_a(args)


def _configure_scratch_paths() -> None:
    value = os.environ.get("MLX_SPATIAL_TEST_SCRATCH")
    if not value:
        raise SystemExit(
            "set MLX_SPATIAL_TEST_SCRATCH to a task-specific directory created with mktemp -d"
        )
    global SCRATCH_ROOT, CACHE_DIR, LOG_PATH
    SCRATCH_ROOT = Path(value)
    CACHE_DIR = SCRATCH_ROOT / "cache/uv-oracle"
    LOG_PATH = SCRATCH_ROOT / "logs/uv-oracle-anchors.log"
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    LOG_PATH.parent.mkdir(parents=True, exist_ok=True)


def _oracle_bootstrap_recipe() -> str:
    venv = SCRATCH_ROOT / "cache/uvoracle-venv"
    return (
        f"python3 -m venv {venv} && {venv / 'bin/pip'} install "
        f"xatlas=={ORACLE_XATLAS_VERSION} numpy"
    )
