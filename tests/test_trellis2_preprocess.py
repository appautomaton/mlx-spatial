...
def test_preprocess_rgb_image_propagates_rmbg_port_blocker(tmp_path):
    ...
    assert not result.ready
    assert result.blocker is not None
    assert result.blocker.operation == "MLX BiRefNet deformable convolution"
    assert "DeformConv2d" in result.blocker.reason


def test_preprocess_rgb_image_blocks_when_rmbg_forward_is_not_implemented(tmp_path):
    image_path = tmp_path / "rgb.png"
    Image.new("RGB", (8, 8), (10, 20, 30)).save(image_path)
    rmbg_root = tmp_path / "rmbg"
    ...
    assert not result.ready
    assert result.blocker is not None
    assert result.blocker.operation == "MLX BiRefNet forward pass"
