...
def test_pixal3d_camera_params_from_moge_intrinsics_match_upstream_formula():
    intrinsics = np.array([[2.5, 0.0, 0.5], [0.0, 2.5, 0.5], [0.0, 0.0, 1.0]], dtype=np.float32)

    camera = pixal3d_camera_params_from_moge_intrinsics(intrinsics, image_width=800)

    expected_fov = 2.0 * np.arctan(800.0 / (2.0 * 2.5 * 800.0))
    assert isinstance(camera, Pixal3DCameraParams)
    np.testing.assert_allclose(camera.camera_angle_x, expected_fov, atol=1e-7)
    assert camera.distance == pixal3d_distance_from_fov(expected_fov)["distance_from_x"]
    assert camera.mesh_scale == 1.0
    assert camera.image_resolution == 512


def test_pixal3d_camera_params_from_moge_intrinsics_validates_inputs():
    valid = np.eye(3, dtype=np.float32)

    for intrinsics, image_width, message in (
        (np.eye(4, dtype=np.float32), 800, "shape"),
        (np.array([[np.nan, 0.0, 0.5], [0.0, 1.0, 0.5], [0.0, 0.0, 1.0]], dtype=np.float32), 800, "finite"),
        (np.array([[0.0, 0.0, 0.5], [0.0, 1.0, 0.5], [0.0, 0.0, 1.0]], dtype=np.float32), 800, "positive"),
        (valid, 0, "image_width"),
    ):
        try:
            pixal3d_camera_params_from_moge_intrinsics(intrinsics, image_width=image_width)
