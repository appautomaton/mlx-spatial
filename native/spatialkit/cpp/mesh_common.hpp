...
inline void validate_vector(nb::object array, const char *name, const char *dtype) {
  const auto ndim = nb::cast<int64_t>(nb::getattr(array, "ndim"));
  if (ndim != 1) {
    std::ostringstream message;
    message << name << " must have rank 1, got rank " << ndim;
    throw nb::value_error(message.str().c_str());
  }
  const std::string actual_dtype = dtype_name(array, name);
  if (actual_dtype != dtype) {
...
inline MeshData load_mesh(nb::object vertices_object, nb::object faces_object) {
  ...
  MeshData mesh;
  mesh.vertices.resize(static_cast<size_t>(vertex_count));
  mesh.faces.resize(static_cast<size_t>(face_count));
  ...
  parallel_for(static_cast<size_t>(vertex_count), [&](size_t begin, size_t end) {
    for (size_t row = begin; row < end; ++row) {
      std::array<float, 3> vertex{
          read_matrix_value<float>(vertex_view, static_cast<int64_t>(row), 0),
          read_matrix_value<float>(vertex_view, static_cast<int64_t>(row), 1),
          read_matrix_value<float>(vertex_view, static_cast<int64_t>(row), 2),
      };
      if (!std::isfinite(vertex[0]) || !std::isfinite(vertex[1]) || !std::isfinite(vertex[2])) {
        invalid_vertex.store(true, std::memory_order_relaxed);
      }
      mesh.vertices[row] = vertex;
    }
  });
  ...
  parallel_for(static_cast<size_t>(face_count), [&](size_t begin, size_t end) {
    for (size_t row = begin; row < end; ++row) {
      std::array<int64_t, 3> face{
          read_matrix_value<int64_t>(face_view, static_cast<int64_t>(row), 0),
          read_matrix_value<int64_t>(face_view, static_cast<int64_t>(row), 1),
          read_matrix_value<int64_t>(face_view, static_cast<int64_t>(row), 2),
      };
      for (int i = 0; i < 3; ++i) {
        if (face[static_cast<size_t>(i)] < 0 || face[static_cast<size_t>(i)] >= vertex_count) {
...
inline nb::object make_float32_array(std::vector<float> values, size_t count) {
  auto owner = new std::vector<float>(std::move(values));
  nb::capsule capsule(owner, [](void *ptr) noexcept {
    delete static_cast<std::vector<float> *>(ptr);
  });
  return nb::ndarray<nb::numpy, float>(owner->data(), {count}, capsule).cast();
}
...
inline nb::object make_int64_array(std::vector<int64_t> values, size_t count) {
  auto owner = new std::vector<int64_t>(std::move(values));
  nb::capsule capsule(owner, [](void *ptr) noexcept {
    delete static_cast<std::vector<int64_t> *>(ptr);
  });
  return nb::ndarray<nb::numpy, int64_t>(owner->data(), {count}, capsule).cast();
}

inline nb::object make_int32_array(std::vector<int32_t> values, size_t count) {
  auto owner = new std::vector<int32_t>(std::move(values));
  nb::capsule capsule(owner, [](void *ptr) noexcept {
    delete static_cast<std::vector<int32_t> *>(ptr);
  });
  return nb::ndarray<nb::numpy, int32_t>(owner->data(), {count}, capsule).cast();
}

inline nb::object make_int32_array(std::vector<int32_t> values, size_t rows, size_t cols) {
  auto owner = new std::vector<int32_t>(std::move(values));
  nb::capsule capsule(owner, [](void *ptr) noexcept {
    delete static_cast<std::vector<int32_t> *>(ptr);
  });
  return nb::ndarray<nb::numpy, int32_t>(owner->data(), {rows, cols}, capsule).cast();
}

inline nb::object make_uint8_array(std::vector<uint8_t> values, size_t count) {
  auto owner = new std::vector<uint8_t>(std::move(values));
  nb::capsule capsule(owner, [](void *ptr) noexcept {
    delete static_cast<std::vector<uint8_t> *>(ptr);
  });
  return nb::ndarray<nb::numpy, uint8_t>(owner->data(), {count}, capsule).cast();
}
...
inline nb::dict mesh_result(const MeshData &mesh) {
  std::vector<float> vertices(mesh.vertices.size() * 3);
  parallel_for(mesh.vertices.size(), [&](size_t begin, size_t end) {
  ...
  std::vector<int64_t> faces(mesh.faces.size() * 3);
  parallel_for(mesh.faces.size(), [&](size_t begin, size_t end) {
    for (size_t index = begin; index < end; ++index) {
      const auto &face = mesh.faces[index];
      faces[index * 3] = face[0];
      faces[index * 3 + 1] = face[1];
      faces[index * 3 + 2] = face[2];
    }
  });
  nb::dict result;
