...
struct RemappedFaceCandidate {
  std::array<int64_t, 3> remapped{};
  std::array<int64_t, 3> canonical{};
};
...
struct FaceEdgeIncidence {
  ...
  int64_t b = 0;
  int64_t face = 0;
  bool canonical_forward = false;
};

struct FaceOrientationResult {
  mesh_common::MeshData mesh;
  int64_t manifold_adjacencies = 0;
  int64_t boundary_edges = 0;
  int64_t nonmanifold_edges = 0;
  int64_t orientation_components = 0;
  int64_t faces_flipped = 0;
  int64_t inconsistent_constraints = 0;
};

class ParityDisjointSet {
 public:
  explicit ParityDisjointSet(size_t size) : parent_(size), parity_to_parent_(size, 0) {
    std::iota(parent_.begin(), parent_.end(), int64_t{0});
  }

  std::pair<int64_t, uint8_t> find(int64_t item) {
    const size_t index = static_cast<size_t>(item);
    if (parent_[index] == item) {
      return {item, 0};
    }
    const auto [root, parent_parity] = find(parent_[index]);
    parity_to_parent_[index] ^= parent_parity;
    parent_[index] = root;
    return {root, parity_to_parent_[index]};
  }

  bool unite(int64_t left, int64_t right, uint8_t required_xor) {
    const auto [left_root, left_parity] = find(left);
    const auto [right_root, right_parity] = find(right);
    if (left_root == right_root) {
      return static_cast<uint8_t>(left_parity ^ right_parity) == required_xor;
    }
    const int64_t low = std::min(left_root, right_root);
    const int64_t high = std::max(left_root, right_root);
    parent_[static_cast<size_t>(high)] = low;
    parity_to_parent_[static_cast<size_t>(high)] =
        static_cast<uint8_t>(left_parity ^ right_parity ^ required_xor);
    return true;
  }

 private:
  std::vector<int64_t> parent_;
  std::vector<uint8_t> parity_to_parent_;
};
...
ClusterResult cluster_mesh(
    const mesh_common::MeshData &input,
    int64_t grid_resolution,
    bool representative_vertices,
    const std::vector<Quadric> *precomputed_vertex_quadrics = nullptr) {
  ...
  result.cpu_workers = static_cast<int64_t>(native_worker_count(input.vertices.size()));
  mesh_common::MeshData output;
  output.vertices.reserve(accumulators.size());
  if (representative_vertices) {
    std::vector<Quadric> owned_vertex_quadrics;
    if (precomputed_vertex_quadrics == nullptr) {
      owned_vertex_quadrics = vertex_plane_quadrics(input);
      precomputed_vertex_quadrics = &owned_vertex_quadrics;
    }
    const std::vector<Quadric> &vertex_quadrics = *precomputed_vertex_quadrics;
    ...
    std::vector<int64_t> candidate_counts(cluster_members.size(), 0);
    std::vector<int64_t> nonfinite_counts(cluster_members.size(), 0);
    std::vector<double> representative_errors(cluster_members.size(), 0.0);
    parallel_for(
        cluster_members.size(),
        [&](size_t begin, size_t end) {
          for (size_t cluster_id = begin; cluster_id < end; ++cluster_id) {
            const ClusterAccum &accum = accumulators[cluster_id];
            ...
            if (best_source_index == std::numeric_limits<size_t>::max()) {
              ...
              if (std::isfinite(best_error)) {
                representative_errors[cluster_id] = best_error;
              }
            }
          }
        },
    ...
    for (size_t cluster_id = 0; cluster_id < cluster_members.size(); ++cluster_id) {
      result.quadric_representative_candidates_evaluated += candidate_counts[cluster_id];
      result.quadric_representative_nonfinite_candidates += nonfinite_counts[cluster_id];
      const double error = representative_errors[cluster_id];
      result.quadric_representative_error_sum += error;
      result.quadric_representative_error_max = std::max(result.quadric_representative_error_max, error);
    }
    output.vertices = std::move(representatives);
  ...
  constexpr size_t kFaceCandidateChunkSize = 8192;
  const size_t face_chunk_count =
      (input.faces.size() + kFaceCandidateChunkSize - 1) / kFaceCandidateChunkSize;
  std::vector<std::vector<RemappedFaceCandidate>> candidates_by_chunk(face_chunk_count);
  std::vector<int64_t> degenerate_by_chunk(face_chunk_count, 0);
  parallel_for(
      input.faces.size(),
      [&](size_t begin, size_t end) {
        const size_t chunk_index = begin / kFaceCandidateChunkSize;
        auto &candidates = candidates_by_chunk[chunk_index];
        candidates.reserve(std::max<size_t>(16, (end - begin) / 8));
        int64_t degenerate_count = 0;
        for (size_t row = begin; row < end; ++row) {
          const auto &face = input.faces[row];
          std::array<int64_t, 3> remapped{
          ...
          candidates.push_back(RemappedFaceCandidate{remapped, canonical});
        }
        degenerate_by_chunk[chunk_index] = degenerate_count;
      },
      2048,
      kFaceCandidateChunkSize);

  size_t candidate_count = 0;
  for (size_t chunk_index = 0; chunk_index < face_chunk_count; ++chunk_index) {
    result.degenerate_faces_removed += degenerate_by_chunk[chunk_index];
    candidate_count += candidates_by_chunk[chunk_index].size();
  }
  std::unordered_set<std::array<int64_t, 3>, FaceKeyHash> seen_faces;
  seen_faces.reserve(candidate_count);
  std::unordered_map<mesh_common::EdgeKey, int64_t, mesh_common::EdgeKeyHash> edge_usage;
  edge_usage.reserve(candidate_count * 3);
  output.faces.reserve(candidate_count);
  for (const auto &chunk : candidates_by_chunk) {
    for (const RemappedFaceCandidate &candidate : chunk) {
      if (!seen_faces.insert(candidate.canonical).second) {
        result.duplicate_faces_removed += 1;
        continue;
      }
      const auto &remapped = candidate.remapped;
...
struct BoundaryCountShard {
  std::mutex mutex;
  std::unordered_map<mesh_common::EdgeKey, int64_t, mesh_common::EdgeKeyHash> counts;
};
...
std::vector<DirectedBoundaryEdge> reference_boundary_edges(const mesh_common::MeshData &mesh) {
  ...
  std::vector<std::unique_ptr<BoundaryCountShard>> shards;
  shards.reserve(shard_count);
  const size_t edges_per_shard = (face_count * 3 + shard_count - 1) / shard_count;
  for (size_t shard = 0; shard < shard_count; ++shard) {
    auto count_shard = std::make_unique<BoundaryCountShard>();
    count_shard->counts.reserve(edges_per_shard);
    shards.push_back(std::move(count_shard));
  }

  constexpr size_t kBoundaryCountChunkSize = 8192;
  parallel_for(
      face_count,
      [&](size_t begin, size_t end) {
        ...
        for (size_t row = begin; row < end; ++row) {
          ...
          for (const auto &edge : edges) {
            const size_t shard = mesh_common::EdgeKeyHash{}(edge) % shard_count;
            local_edges[shard].push_back(edge);
          }
        }
        for (size_t shard = 0; shard < shard_count; ++shard) {
          if (local_edges[shard].empty()) {
            continue;
          }
          BoundaryCountShard &count_shard = *shards[shard];
          std::lock_guard<std::mutex> lock(count_shard.mutex);
          for (const auto &edge : local_edges[shard]) {
            count_shard.counts[edge] += 1;
          }
        }
      },
      2048,
      kBoundaryCountChunkSize);

  std::vector<std::array<uint8_t, 3>> boundary_flags(face_count, {0, 0, 0});
  parallel_for(face_count, [&](size_t begin, size_t end) {
    for (size_t row = begin; row < end; ++row) {
      ...
      for (size_t corner = 0; corner < 3; ++corner) {
        const auto &edge = edges[corner];
        const size_t shard = mesh_common::EdgeKeyHash{}(edge) % shard_count;
        const auto found = shards[shard]->counts.find(edge);
        boundary_flags[row][corner] =
            static_cast<uint8_t>(found != shards[shard]->counts.end() && found->second == 1);
      }
    }
  });

  std::vector<DirectedBoundaryEdge> boundary_edges;
  for (size_t row = 0; row < face_count; ++row) {
    const auto &face = mesh.faces[row];
    ...
    for (size_t corner = 0; corner < 3; ++corner) {
      if (boundary_flags[row][corner] != 0) {
        boundary_edges.push_back(directed_edges[corner]);
      }
    }
  }
...
FaceOrientationResult unify_reference_face_orientations(
    const mesh_common::MeshData &input) {
  FaceOrientationResult result;
  result.mesh = input;
  const size_t face_count = input.faces.size();
  if (face_count == 0) {
    return result;
  }

  std::vector<FaceEdgeIncidence> incidences(face_count * 3);
  parallel_for(face_count, [&](size_t begin, size_t end) {
    for (size_t face_id = begin; face_id < end; ++face_id) {
      const auto &face = input.faces[face_id];
      for (size_t local_edge = 0; local_edge < 3; ++local_edge) {
        const int64_t start = face[local_edge];
        const int64_t finish = face[(local_edge + 1) % 3];
        incidences[face_id * 3 + local_edge] = FaceEdgeIncidence{
            std::min(start, finish),
            std::max(start, finish),
            static_cast<int64_t>(face_id),
            start < finish,
        };
      }
    }
  ...
  std::sort(
      incidences.begin(),
      incidences.end(),
      [](const FaceEdgeIncidence &left, const FaceEdgeIncidence &right) {
        if (left.a != right.a) {
  ...
  ParityDisjointSet components(face_count);
  size_t group_begin = 0;
  while (group_begin < incidences.size()) {
    size_t group_end = group_begin + 1;
    while (group_end < incidences.size()
           && incidences[group_end].a == incidences[group_begin].a
           && incidences[group_end].b == incidences[group_begin].b) {
      ++group_end;
    }
    ...
    if (group_size == 1) {
      ...
      const FaceEdgeIncidence &left = incidences[group_begin];
      const FaceEdgeIncidence &right = incidences[group_begin + 1];
      const uint8_t required_xor = left.canonical_forward == right.canonical_forward ? 1 : 0;
      if (!components.unite(left.face, right.face, required_xor)) {
        result.inconsistent_constraints += 1;
      }
      result.manifold_adjacencies += 1;
    } else {
  ...
  std::vector<uint8_t> flip_flags(face_count, 0);
  std::unordered_set<int64_t> roots;
  roots.reserve(face_count);
  for (size_t face_id = 0; face_id < face_count; ++face_id) {
    const auto [root, parity] = components.find(static_cast<int64_t>(face_id));
    roots.insert(root);
    flip_flags[face_id] = parity;
    result.faces_flipped += parity != 0 ? 1 : 0;
  }
  result.orientation_components = static_cast<int64_t>(roots.size());
  parallel_for(face_count, [&](size_t begin, size_t end) {
    for (size_t face_id = begin; face_id < end; ++face_id) {
      if (flip_flags[face_id] != 0) {
        std::swap(result.mesh.faces[face_id][1], result.mesh.faces[face_id][2]);
      }
    }
  });
  return result;
...
nb::dict build_qem_stats(
  ...
  stats["cluster_attempt_count"] = 0;
  stats["cluster_attempt_seconds"] = 0.0;
  stats["cluster_quadric_build_seconds"] = 0.0;
  ...
  stats["qem_initialization_seconds"] = qem.initialization_seconds;
  stats["qem_collapse_seconds"] = qem.collapse_seconds;
  stats["framework"] = "native-c++";
  stats["execution_device"] = "cpu";
  stats["cpu_workers"] = static_cast<int64_t>(native_worker_count(static_cast<size_t>(pre_simplify_faces)));
  return stats;
}
...
nb::dict fill_holes(
    nb::object vertices,
    nb::object faces,
    double max_hole_perimeter) {
  if (!std::isfinite(max_hole_perimeter) || max_hole_perimeter <= 0.0) {
    throw nb::value_error("max_hole_perimeter must be positive and finite");
  }
  mesh_common::MeshData input = mesh_common::load_mesh(vertices, faces);
  nb::gil_scoped_release release;
  const ReferenceLoopFillResult fill = fill_reference_clean_boundary_loops(
      input,
      std::numeric_limits<int64_t>::max(),
      max_hole_perimeter);
  const int64_t residual_clean_boundary_loops =
      count_residual_clean_boundary_loops(fill.mesh);
  const int64_t cpu_workers = static_cast<int64_t>(native_worker_count(input.faces.size()));
  nb::gil_scoped_acquire acquire;
  nb::dict result = mesh_common::mesh_result(fill.mesh);
  nb::dict stats;
  stats["backend"] = "native-cpp";
  stats["framework"] = "native-c++";
  stats["execution_device"] = "cpu";
  stats["cpu_workers"] = cpu_workers;
  stats["algorithm"] = kSmallBoundaryLoopFillAlgorithm;
  stats["max_hole_perimeter"] = max_hole_perimeter;
  stats["source_vertices"] = static_cast<int64_t>(input.vertices.size());
  stats["source_faces"] = static_cast<int64_t>(input.faces.size());
  stats["final_vertices"] = static_cast<int64_t>(fill.mesh.vertices.size());
  stats["final_faces"] = static_cast<int64_t>(fill.mesh.faces.size());
  stats["boundary_edges_before"] = fill.boundary_edges_before;
  stats["clean_boundary_loops"] = fill.clean_boundary_loops;
  stats["filled_loops"] = fill.filled_loops;
...
nb::dict unify_face_orientations(
    nb::object vertices,
    nb::object faces) {
  mesh_common::MeshData input = mesh_common::load_mesh(vertices, faces);
  nb::gil_scoped_release release;
  FaceOrientationResult oriented = unify_reference_face_orientations(input);
  const int64_t cpu_workers = static_cast<int64_t>(native_worker_count(input.faces.size()));
  nb::gil_scoped_acquire acquire;
  nb::dict result = mesh_common::mesh_result(oriented.mesh);
  nb::dict stats;
  stats["backend"] = "native-cpp";
  stats["framework"] = "native-c++";
  stats["execution_device"] = "cpu";
  stats["cpu_workers"] = cpu_workers;
  stats["algorithm"] = "cumesh-manifold-adjacency-parity";
  stats["source_vertices"] = static_cast<int64_t>(input.vertices.size());
  stats["source_faces"] = static_cast<int64_t>(input.faces.size());
  stats["final_vertices"] = static_cast<int64_t>(oriented.mesh.vertices.size());
  stats["final_faces"] = static_cast<int64_t>(oriented.mesh.faces.size());
  stats["manifold_adjacencies"] = oriented.manifold_adjacencies;
  stats["boundary_edges"] = oriented.boundary_edges;
  stats["nonmanifold_edges"] = oriented.nonmanifold_edges;
  stats["orientation_components"] = oriented.orientation_components;
  stats["faces_flipped"] = oriented.faces_flipped;
  stats["inconsistent_constraints"] = oriented.inconsistent_constraints;
  stats["orientation_consistent"] = oriented.inconsistent_constraints == 0;
  result["stats"] = stats;
  return result;
}
...
nb::dict simplify_mesh(
  ...
  if (selection.is_qem) {
    ...
    const auto collapse_started = std::chrono::steady_clock::now();
    QemSimplifyResult qem = simplifier.run(target_faces);
    const auto collapse_finished = std::chrono::steady_clock::now();
    qem.initialization_seconds =
        std::chrono::duration<double>(collapse_started - initialization_started).count();
    qem.collapse_seconds = std::chrono::duration<double>(collapse_finished - collapse_started).count();
    nb::gil_scoped_acquire acquire;
  ...
  if (static_cast<int64_t>(input.faces.size()) <= target_faces) {
    ...
    stats["framework"] = "native-c++";
    stats["execution_device"] = "cpu";
    stats["cpu_workers"] = static_cast<int64_t>(native_worker_count(input.faces.size()));
    result["stats"] = stats;
    return result;
  }
  ...
  std::vector<Quadric> cluster_vertex_quadrics;
  double cluster_quadric_build_seconds = 0.0;
  if (selection.topology_aware) {
    const auto started = std::chrono::steady_clock::now();
    cluster_vertex_quadrics = vertex_plane_quadrics(*simplification_input);
    cluster_quadric_build_seconds =
        std::chrono::duration<double>(std::chrono::steady_clock::now() - started).count();
  }
  const std::vector<Quadric> *cluster_vertex_quadrics_ptr =
      selection.topology_aware ? &cluster_vertex_quadrics : nullptr;
  int64_t cluster_attempt_count = 0;
  double cluster_attempt_seconds = 0.0;
  auto run_cluster_attempt = [&](int64_t resolution) {
    const auto started = std::chrono::steady_clock::now();
    ClusterResult attempt =
        cluster_mesh(*simplification_input, resolution, selection.topology_aware, cluster_vertex_quadrics_ptr);
    cluster_attempt_seconds +=
        std::chrono::duration<double>(std::chrono::steady_clock::now() - started).count();
    cluster_attempt_count += 1;
    return attempt;
  };

  int64_t grid_resolution = initial_grid_resolution(target_faces);
  ClusterResult best = run_cluster_attempt(grid_resolution);
  for (int attempt = 0; attempt < 4; ++attempt) {
    ...
    ClusterResult candidate = run_cluster_attempt(grid_resolution);
  ...
  stats["cluster_attempt_count"] = cluster_attempt_count;
  stats["cluster_attempt_seconds"] = cluster_attempt_seconds;
  stats["cluster_quadric_build_seconds"] = cluster_quadric_build_seconds;
  stats["framework"] = "native-c++";
  stats["execution_device"] = "cpu";
  stats["cpu_workers"] = best.cpu_workers;
  result["stats"] = stats;
  return result;
}
