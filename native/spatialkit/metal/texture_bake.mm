...
NSString *texture_bake_resource_path(NSString *resource_name) {
  Dl_info info;
  if (dladdr(reinterpret_cast<const void *>(&metal_device_available), &info) == 0 || info.dli_fname == nullptr) {
    throw std::runtime_error("failed to locate mlx-spatial native module for Metal resource loading");
  }
  NSString *module_path = [NSString stringWithUTF8String:info.dli_fname];
  if (module_path == nil) {
    throw std::runtime_error("failed to convert mlx-spatial native module path for Metal resource loading");
  }
  return [[module_path stringByDeletingLastPathComponent] stringByAppendingPathComponent:resource_name];
}

id<MTLLibrary> load_texture_bake_library(id<MTLDevice> device) {
  NSError *error = nil;
  NSString *metallib_path = texture_bake_resource_path(@"texture_bake.metallib");
  if ([[NSFileManager defaultManager] fileExistsAtPath:metallib_path]) {
    id<MTLLibrary> library = [device newLibraryWithURL:[NSURL fileURLWithPath:metallib_path] error:&error];
    if (library != nil) {
      return library;
    }
  }
  ...
  error = nil;
  NSString *source_path = texture_bake_resource_path(@"texture_bake.metal");
  NSString *source = [NSString stringWithContentsOfFile:source_path encoding:NSUTF8StringEncoding error:&error];
  if (source == nil) {
    throw metal_error("failed to load mlx-spatial texture_bake.metal source", error);
  }
  error = nil;
  id<MTLLibrary> library = [device newLibraryWithSource:source options:nil error:&error];
  if (library == nil) {
    throw metal_error("failed to compile mlx-spatial texture_bake.metal source", error);
  }
  return library;
}

std::runtime_error metal_error(const std::string &message, NSError *error) {
  if (error == nil) {
...
ReferenceSampleStats apply_reference_projection_and_trilinear_sampling(
  ...
  constexpr size_t kProjectionChunkSize = 256;
  std::vector<ReferenceSampleStats> partials((texel_count + kProjectionChunkSize - 1) / kProjectionChunkSize);
  parallel_for(
      texel_count,
      [&](size_t begin, size_t end) {
        ReferenceSampleStats local;
        for (size_t texel = begin; texel < end; ++texel) {
          if (coverage[texel] == 0) {
            continue;
          }
          local.uv_surface_texels += 1;
          ...
          if (projected.face_index < 0) {
            ...
            local.trilinear_invalid_texels += 1;
            continue;
          }
          local.projected_texels += 1;
          local.max_projection_distance =
              std::max(local.max_projection_distance, std::sqrt(projected.distance2));
          std::array<float, 6> sampled{};
          ...
          const bool valid = trilinear_sample_attributes(
              ...
              &out_of_grid_corner);
          if (missing_corner) {
            local.trilinear_missing_corner_texels += 1;
          }
          if (out_of_grid_corner) {
            local.trilinear_out_of_grid_corner_texels += 1;
          }
          if (!valid) {
            ...
            if (fallback_valid) {
              ...
              local.nearest_fallback_sampled_texels += 1;
              continue;
            }
            local.nearest_fallback_missing_texels += 1;
          ...
          coverage[texel] = 1;
          write_pbr_texel_cpu(sampled, texel, base_color, metallic_roughness);
          local.trilinear_sampled_texels += 1;
        }
        partials[begin / kProjectionChunkSize] = local;
      },
      2048,
      kProjectionChunkSize);
  for (const auto &partial : partials) {
    stats.uv_surface_texels += partial.uv_surface_texels;
    stats.projected_texels += partial.projected_texels;
    stats.trilinear_sampled_texels += partial.trilinear_sampled_texels;
    stats.trilinear_missing_corner_texels += partial.trilinear_missing_corner_texels;
    stats.trilinear_out_of_grid_corner_texels += partial.trilinear_out_of_grid_corner_texels;
    stats.trilinear_invalid_texels += partial.trilinear_invalid_texels;
    stats.nearest_fallback_sampled_texels += partial.nearest_fallback_sampled_texels;
    stats.nearest_fallback_missing_texels += partial.nearest_fallback_missing_texels;
    stats.max_projection_distance = std::max(stats.max_projection_distance, partial.max_projection_distance);
  }
  stats.cpu_workers = static_cast<int64_t>(native_worker_count(texel_count));
  return stats;
}
...
nb::dict bake_pbr_texture_metal(
  ...
  if (texture_size > kMaxTextureDimension) {
    throw nb::value_error("texture_size exceeds mlx-spatial SpatialKit Metal texture dimension guard");
  }
  ...
  std::vector<VoxelRecord> records = load_voxels(
      ...
      &grid_y,
      &grid_x);
  nb::gil_scoped_release native_release;
  ...
  @autoreleasepool {
    id<MTLDevice> device = MTLCreateSystemDefaultDevice();
    if (device == nil) {
      throw std::runtime_error("Metal device unavailable for mlx-spatial SpatialKit texture bake");
    }

    NSError *error = nil;
    id<MTLLibrary> library = load_texture_bake_library(device);
    ...
    if (pipeline == nil) {
      throw metal_error("failed to create mlx-spatial SpatialKit texture bake Metal pipeline", error);
    }
    id<MTLCommandQueue> queue = [device newCommandQueue];
    if (queue == nil) {
      throw std::runtime_error("failed to create mlx-spatial SpatialKit Metal command queue");
    }
    ...
    id<MTLBuffer> vertex_buffer = make_buffer(device, vertices.data(), vertices.size() * sizeof(float), @"mlx-spatial SpatialKit vertices");
    id<MTLBuffer> face_buffer = make_buffer(device, faces.data(), faces.size() * sizeof(int32_t), @"mlx-spatial SpatialKit faces");
    id<MTLBuffer> uv_buffer = make_buffer(device, uvs.data(), uvs.size() * sizeof(float), @"mlx-spatial SpatialKit uvs");
    id<MTLBuffer> key_buffer = make_buffer(device, keys.data(), keys.size() * sizeof(uint64_t), @"mlx-spatial SpatialKit voxel keys");
    id<MTLBuffer> attr_buffer = make_buffer(device, attributes.data(), attributes.size() * sizeof(float), @"mlx-spatial SpatialKit voxel attributes");
    id<MTLBuffer> config_buffer = make_buffer(device, &config, sizeof(BakeConfig), @"mlx-spatial SpatialKit bake config");
    id<MTLBuffer> base_buffer = make_output_buffer(device, base_bytes, @"mlx-spatial SpatialKit base color");
    id<MTLBuffer> mr_buffer = make_output_buffer(device, mr_bytes, @"mlx-spatial SpatialKit metallic roughness");
    id<MTLBuffer> coverage_buffer = make_output_buffer(device, coverage_bytes, @"mlx-spatial SpatialKit coverage");
    id<MTLBuffer> surface_position_buffer =
        make_output_buffer(device, surface_position_bytes, @"mlx-spatial SpatialKit UV surface positions");
    id<MTLBuffer> uv_bin_offsets_buffer = make_buffer(
        device,
        uv_bins.offsets.data(),
        uv_bins.offsets.size() * sizeof(uint32_t),
        @"mlx-spatial SpatialKit UV bin offsets");
    id<MTLBuffer> uv_bin_faces_buffer = make_buffer(
        device,
        uv_bins.faces.data(),
        uv_bins.faces.size() * sizeof(int32_t),
        @"mlx-spatial SpatialKit UV bin faces");

    id<MTLCommandBuffer> command_buffer = [queue commandBuffer];
    if (command_buffer == nil) {
      throw std::runtime_error("failed to create mlx-spatial SpatialKit Metal command buffer");
    }
    id<MTLComputeCommandEncoder> encoder = [command_buffer computeCommandEncoder];
    if (encoder == nil) {
      throw std::runtime_error("failed to create mlx-spatial SpatialKit Metal command encoder");
    }
    ...
    if ([command_buffer error] != nil) {
      throw metal_error("mlx-spatial SpatialKit Metal texture bake command failed", [command_buffer error]);
    }
    ...
    stats["framework"] = "metal";
    stats["execution_device"] = "apple-gpu";
    stats["gpu_submission_confirmed"] = true;
    ...
    stats["source_projection_cpu_workers"] = reference_sample_stats.cpu_workers;
